...
class Test_CREATE_User:
    ...
    def test_create_user(self):
        ...
        c = client.Client()
        response = c.post(self.PATH,
                          data=data,
                          content_type='application/json')

        assert response.status_code == 201

    # (CREATE)既に登録されているユーザー名で登録しようとした場合にエラーが返ってくることを確認するテスト
    @pytest.mark.django_db
...
class Test_UPDATE_User:
    ...
    # (UPDATE)ユーザー情報の更新が正常に行われることを確認するテスト
    @pytest.mark.skip
    @pytest.mark.django_db
...
class Test_Login:
    ...
    # (LOGIN)ログインが正しいユーザーならばログインが成功することを確認するテスト
    @pytest.mark.django_db
    ...
    # (LOGIN)誤ったユーザならばログインが失敗することを確認するテスト
    @pytest.mark.django_db
    ...
    # (LOGIN)誤ったパスワードならばログインが失敗することを確認するテスト
    @pytest.mark.django_db
